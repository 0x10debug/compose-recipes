...
- **Gitea Actions runner** (`--profile actions`): the built-in CI for Gitea, an alternative to Woodpecker. Generate a runner token in Gitea (*Site administration → Actions → Runners → Create runner token*), put it in `GITEA_RUNNER_REGISTRATION_TOKEN` in `.env`, then start the suite together: `docker compose --profile all --profile actions up -d` (the runner's dependency on Gitea is optional so the profile validates standalone, but jobs need Gitea running). The runner registers itself with the instance URL on first start. Jobs execute in containers through the host Docker socket; for least privilege, deploy the [socket-proxy](../socket-proxy/) suite and point `DOCKER_HOST=tcp://socket-proxy:2375` instead of mounting the raw socket. Memory and CPU are capped (2 GB / 1.5 CPU) because runners execute arbitrary workloads.
- **Backups**: the Gitea and Woodpecker PostgreSQL databases carry `backup.hook` labels, so [backup-kit](https://github.com/0x10debug/backup-kit) (or any runner of `lib/backup-hooks.sh`) dumps them before file backups and cleans up afterwards - see `docs/backup-hooks.md`. Volumes under `/data/` are picked up by file-level backups directly.

## Scope note

This suite intentionally covers the whole GitOps toolchain (forge, CI, registry) instead of shipping a separate `gitops` suite: Gitea + Woodpecker + Registry + a Gitea Actions runner already compose a complete self-hosted CI/CD stack, and a second suite would duplicate it. If you need only CI without the dev tools, disable profiles you do not use.
